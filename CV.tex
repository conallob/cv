...
\item Technologies:
  ...
  \item AI Agents: Claude, Claude Code, Windsurf, ChatGPT
...
\begin{itemize}[noitemsep]
	\item Overhauled SRE onboarding, replacing tribal knowledge with structured documentation, recorded talks \& a peer-forum, measurably reducing ramp time for every subsequent SRE hire
	\item Designed \& implemented a managed, multi-environment synthetic observability platform (Cloudprober) spanning VM control plane, CrusoeCloud, Kubernetes \& bare-metal fleet; onboarded 8+ internal teams as users
	\item Became the observability \& PromQL/MetricQL expert across SRE, engineering \& Customer Success; presented Cloudprober to the company-wide Cloud Availability Forum
	...
	\item Joined SDN oncall embedded rotation; identified \& fixed repeating oncall issues, and landed 5x--10x scaling of the SDN control plane through load balancer tuning, horizontal pod autoscaling \& targeted Kubernetes optimisations across all production locations
	\item Built CI/CD pipeline via GoReleaser for the internal Cloudprober platform; contributed upstream fixes to open-source Cloudprober (\texttt{github.com/cloudprober/cloudprober})
    \item Closed the automation engine observability gap by publishing both Ansible/AWX playbook execution history \& fleet-wide node metadata as queryable metrics; resulting impacts included reducing detection time of automation-executed changes during incidents from O(hours) to O(minutes), \& reducing non-actionable pages to oncallers by 90+\%
\end{itemize}
...
\begin{itemize}[noitemsep]
  \item Qualified \& rolled out 2nd generation IPv6 Load Balancing, enabling World IPv6 Day \& World IPv6 Launch.
	...
	\item Designed \& implemented a Cluster Health Console; a cross-cutting RAG status dashboard spanning all Google infrastructure within a cluster, used by Incident Commanders to debug low-level issues \& inform decisions such as whether to EPO a location
\end{itemize}
